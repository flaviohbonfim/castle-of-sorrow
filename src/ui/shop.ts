...
export class ShopUI {
  ...
  private buy(game: Game, entry: ShopEntry | undefined): void {
    if (!entry) return;
    const p = game.player;
    if (p.inventory.gold < entry.price) {
      audio.play("hurt");
      return;
    }
    p.inventory.gold -= entry.price;
    if (entry.relicId) {
  ...
  draw(ctx: CanvasRenderingContext2D, game: Game): void {
    ...
    ctx.fillText("— HERMIT'S WARES —", 76, 46);
    ctx.textAlign = "right";
