...
export class Minimap {
  draw(ctx: CanvasRenderingContext2D, currentRoom: string, flags: Set<string>, tick: number): void {
    // Bounds of the full castle grid.
    let maxGx = 0;
    let maxGy = 0;
    for (const def of Object.values(ROOMS)) {
      maxGx = Math.max(maxGx, def.mapRect.gx + def.mapRect.gw);
      maxGy = Math.max(maxGy, def.mapRect.gy + def.mapRect.gh);
    }
    const originX = VIEW_W - 10 - maxGx * CELL;
    const originY = 32;

    ctx.save();
    ctx.globalAlpha = 0.85;
    ctx.fillStyle = "rgba(10, 6, 20, 0.7)";
    ctx.fillRect(originX - 3, originY - 3, maxGx * CELL + 6, maxGy * CELL + 6);
